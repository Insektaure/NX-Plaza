...
| **Blessings** | Every fifth floor offers three out of eighteen, and the one you take is yours for the rest of the climb. Six of them - the ones that only move a number - can come round again, and each copy adds the same again: four Whetstones are +80%, not ×2.07. Each can be taken five times, Hale ten, so they run out around floor two hundred. The party never hits for less than a quarter or takes less than a quarter, whatever the mix. |
...
background. Every one is synthesised rather than loaded from a file, so the
whole sound bank costs the `.nro` nothing: a note is a struck harmonic series
